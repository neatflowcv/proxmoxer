...
description: "Use this agent when the user requests a complete code implementation workflow that involves writing code, reviewing it, and iterating on feedback. This includes requests for new features, refactoring, bug fixes, or any substantial code changes that benefit from a structured development and review cycle.\\n\\nExamples:\\n\\n<example>\\nContext: User requests a new feature implementation that requires coding and review.\\nuser: \"새로운 API 엔드포인트를 추가해줘\"\\nassistant: \"코딩 오케스트레이터 에이전트를 사용하여 체계적인 개발 및 리뷰 프로세스를 진행하겠습니다.\"\\n<commentary>\\nSince this is a substantial code implementation request, use the Task tool to launch the coding-orchestrator agent to coordinate the development workflow.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: User wants to implement and review a complex feature.\\nuser: \"도메인 서비스에 캐싱 로직을 구현하고 리뷰해줘\"\\nassistant: \"Task 도구를 사용하여 coding-orchestrator 에이전트를 실행하겠습니다. 이 에이전트가 go-developer로 구현하고, code-reviewer로 리뷰한 뒤, 중요 이슈를 수정하는 전체 워크플로우를 관리합니다.\"\\n<commentary>\\nThis request explicitly asks for both implementation and review, making it ideal for the coding-orchestrator agent.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: User asks for a bug fix with quality assurance.\\nuser: \"클러스터 동기화 버그 수정해줘\"\\nassistant: \"coding-orchestrator 에이전트를 통해 버그 수정을 진행하겠습니다. go-developer가 수정하고, code-reviewer가 검토한 후, 중요 이슈가 있으면 다시 수정하는 프로세스를 따릅니다.\"\\n<commentary>\\nBug fixes benefit from the structured review process to ensure the fix doesn't introduce new issues.\\n</commentary>\\n</example>"
model: haiku
color: red
---
